...
const { CulturePage } = require('../pages/culturepage.page');
test.describe('Culture Page', () => {
    test.use({
    ...
    test('TC_01 : Validate culture Page Loads Successfully', async ({ page }) => {
        const cultureObj = new CulturePage(page);
        await cultureObj.culturePageLink.click();

        await expect(page).toHaveURL(/culture/, { timeout: 30000 });
    })

    test('TC_02 : Validate culture Page Section Labels', async ({ page }) => {
        const cultureObj = new CulturePage(page);
        await cultureObj.culturePageLink.click();
        await page.waitForTimeout(6000);
        ...
        for (let i = 0; i < count; i++) {
            ...
            await expect(label).toBeVisible({duration:15000});
        }
    });

    test('TC_03 : Validate culture page sub-category navigation', async ({ page }) => {
    ...
    test('TC_04 : Validate culture page Sticky header subcategory navigation', async ({ page }) => {
        ...
        for (let i = 0; i < stickyHeaderMenuCount; i++) {
            ...
            const cleanedTitle = (await menuPageTitle.textContent())
                .replace(/[\n.]/g, ' ')
    ...
    test('TC_05 : Verify culture Page Articles navigation and load successfully', async ({ page }) => {
